...
// SF6015 UART
HardwareSerial sf(1);
const int SF_RX = D3; // D3 (RX)  <- driver TX (via 5V->3.3V divider/level shifter)
const int SF_TX = D2; // D2 (TX)  -> driver RX
...
void bringUpAndStartSF(float amps) {
  // Configure NTC protection on pin 14
  sf_setNTC_Beta(0x0F94);            // β = 3988
  sf_setNTC_Limits(0.0f, 40.0f);     // change to 40.0f if you want 0–40 °C

  // 1) Internal current set
  sendCmdSF("P0700 0020"); delay(40);

  // 2) Internal enable
  sendCmdSF("P0700 0400"); delay(40);

  // 3) Allow interlock (hardware interlock pin must be at GND)
  sendCmdSF("P0700 1000"); delay(60);

  // 4) Allow NTC interlock (so external NTC can trip protection)
  sendCmdSF("P0700 8000"); delay(60);

  // Set target current
  setIsetAmps(amps);

  // 5) START — must be last; do not touch P0700 after this (except keep-alive START)
  sendCmdSF("P0700 0008"); delay(80);
}
...
HardwareSerial tc(2);
const int TC_RX = D5; // D8 (Arduino RX)  <- TC1540 TX
const int TC_TX = D4; // D7 (Arduino TX)  -> TC1540 RX
