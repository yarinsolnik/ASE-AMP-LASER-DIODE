// Nano ESP32 ↔ SF6015 (0–10 A via potentiometer) + SH1107 128x128 I2C OLED
...
const float I_MAX = 4.0f;  // max set current 10 A
...
bool readLineSF(String& out, unsigned long timeout_ms = 20) {
  unsigned long t0 = millis();
  while (millis() - t0 < timeout_ms) {
    if (sf.available()) { out = sf.readStringUntil('\r'); out.trim(); return true; }
  }
  return false;
}
...
// --- ADDED: SF6015 NTC interlock helpers (pin 14 of analog connector) ---
...
static void sf_enableNTCInterlock(bool enable) {
  uint16_t state = 0;
  // read-modify-write to preserve your other bits
  if (!readParamSF("0700", state)) state = 0;
  if (enable) {
    state |=  0x8000;   // Allow NTC interlock
    state &= ~0x4000;   // Clear Deny NTC interlock
  } else {
    state |=  0x4000;   // Deny NTC interlock
    state &= ~0x8000;   // Clear Allow
  }
  char buf[24];
  snprintf(buf, sizeof(buf), "P0700 %04X", state);
  sendCmdSF(buf); delay(40);
}
// --- END ADDED ---

// Bring-up sequence (START must be last)
void bringUpAndStartSF(float amps) {
  sendCmdSF("P0700 0020"); delay(40);  // Internal current-set
  sendCmdSF("P0700 0400"); delay(40);  // Internal enable
  sendCmdSF("P0700 1000"); delay(60);  // Allow interlock (hardware interlock pin must be at GND)

  // --- ADDED: configure on-board NTC protection on pin 14 ---
  sf_setNTC_Beta(0x0F6E);              // β = 3950
  sf_setNTC_Limits(0.0f, 40.0f);       // 0.0 °C .. 40.0 °C
  sf_enableNTCInterlock(true);         // enable NTC interlock
  // --- END ADDED ---

  setIsetAmps(amps);                   // Set current
  sendCmdSF("P0700 0008"); delay(80);  // START
}
...
HardwareSerial tc(2);
// UART pins for TC1540 (adjust if you used others)
const int TC_RX = D9; // D8 (Arduino RX)  <- TC1540 TX (via 5V->3.3V divider/level shifter)
const int TC_TX = D8; // D7 (Arduino TX)  -> TC1540 RX

// Fixed temperature setpoint
const float TEC_FIXED_SET_C = 25.00f;
const uint32_t TEC_REASSERT_MS = 3000;  // periodically re-assert setpoint
...
void setTempSourceExternalNTC() {
  ...
  // Clear bit1 (temperature source internal) → use external sensor (NTC)
  state &= ~0x0002;
  writeStateTC(state);
}

void bringUpAndStartTC(float tempC) {
  // Internal setpoint + internal enable + allow interlock
  writeStateTC(0x0020); // Internal temperature SET (setpoint source = UART/internal)
  writeStateTC(0x0400); // Internal enable
  writeStateTC(0x1000); // Allow interlock
  // Force feedback source = external NTC
...
display.setCursor(0, y); display.print("SOLNIK Photonics"); y += 12;
...
void setup() {
  ...
  // OLED init (D11 SDA, D12 SCL on Nano ESP32)
  Wire.begin(D6, D7);
...
void loop() {
  ...
  if (!startedLaser) bringUpAndStartSF(lastSentA); // keep-alive
  ...
  // Self-heal / reassert: make sure we keep external NTC and 25C
  if (!startedTEC || ilDenyTEC || tempInt || !enInt ||
    ...
    setTempSourceExternalNTC();      // ensure external NTC
    bringUpAndStartTC(TEC_FIXED_SET_C);
    lastTECAssertMs = now;
  }
  ...
  drawDashboard(interlocks,
                ...
                (isnan(Tmeas) ? 0 : Tmeas));  // Tmeas now from external NTC
